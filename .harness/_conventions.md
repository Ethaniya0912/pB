...
## 17. `/test-run <cycle>` — 테스트 씬 자동 셋업·플레이 검증
사이클이 정의한 **씬/프리팹/Hierarchy/월드 셋팅**을 실제 Unity 에 재현해, 에셋이 잘 불러와지는지 플레이로
검증하는 별도 진입점(Skill `test-run`). 분석(9단계)과 분리된 **재현·검증 단계**다.

### 17-A. 전제 — 셋팅 명시 필수
대상 사이클의 **④ assets·⑥ test_env 에 씬·프리팹·Hierarchy·월드 셋팅이 구체적으로 명시**돼 있어야 한다
(어느 씬에 어떤 GO 가 어떤 부모 밑에, 어떤 컴포넌트·필드값으로). 명시가 부족하거나 "런타임 자동생성"
뿐인 사이클이면, `/test-run` 은 먼저 **그 갭을 사용자에게 질문**하고 멈춘다(임의 추측 금지).

### 17-B. 경로 (결정)
| 무엇 | 위치 | 비고 |
|---|---|---|
| 테스트 문서 | `cycles/<cycle>/test_run/` | `test_def.md`(상세 정의)·`asset_map.md`(에셋 매핑)·`result.md`·`evidence/` |
| 테스트 에셋 | `Assets/_TestRuns/<cycle>/` | Unity 가 임포트해야 하므로 **Assets/ 안**. `_` 접두로 최상단 정렬·빌드 제외. 하위 `assets/` 에 사이클이 정의한 전체 디렉토리 구조(더미든 실제든) |
| 테스트 씬 | `Assets/_TestRuns/<cycle>/<cycle>_TestScene.unity` | **빌드 씬 목록에 넣지 않는다**(테스트 전용) |
- `_TestRuns/` 는 git 추적하되 빌드 산출에서 제외(빌드 씬 미포함 + 필요 시 `.gitignore` 선택). Assets 밖 하네스 규약(§1)의 예외 — 테스트 에셋은 실제 임포트 대상이라 Assets/ 안이어야 한다.

### 17-C. 더미 에셋 (결정: 동작 stub)
없는 에셋은 **동작 stub** 으로 자동 생성한다 — 단순 placeholder 가 아니라 로딩·참조·플레이까지 검증되게:
- 프리팹: 사이클이 명시한 컴포넌트(있으면 실제 타입, 없으면 placeholder MonoBehaviour)·메시(primitive)·머티리얼 더미를 부착해 저장.
- 머티리얼: `new Material(Shader.Find("Universal Render Pipeline/Lit"))` 등 프로젝트 기본 셰이더로.
- ScriptableObject/Data: 사이클이 타입을 명시했고 존재하면 `CreateInstance`, 아니면 placeholder.
- 더미에는 식별 마커(이름 접미 `__DUMMY` 또는 주석/라벨)를 남겨 실제 에셋과 구분한다.

### 17-D. 더미 ↔ 실제 교체 (결정: 둘 다 지원)
1. **매핑 매니페스트** — `test_run/asset_map.md` 의 슬롯별 표에서 `현재=더미|실제`·실제 경로를 지정.
   `/test-run` 재실행 시 이 표를 읽어 실제 경로가 있으면 링크, 없으면 더미 생성.
2. **폴더 드롭** — `Assets/_TestRuns/<cycle>/assets/` 의 더미와 **같은 상대경로·이름**으로 실제 에셋을 넣으면
   재실행 시 더미를 덮지 않고 실제를 사용(스캐폴드는 기존 파일 보존). 교체 후 매핑표가 자동 갱신된다.

### 17-E. 자동 수정 경계 (결정: 테스트 환경·매핑만)
플레이 실패 시 **테스트 환경·매핑 오류만** 자동 수정한다 — 씬 배치 누락·더미 미생성·경로 오타·참조 미연결·
컴포넌트 미부착 등. **게임 본 코드(`Assets/Scripts/...`)는 고치지 않고** 원인으로 의심되면 G6(설계 분기)로
사용자에게 보고만 한다(테스트 하네스가 프로덕트 코드를 임의 변경하지 않는다는 안전선).

### 17-F. 절차·산출 (상세는 skill `test-run`)
문서 읽기(셋팅 추출) → 스캐폴드 → `test_def.md`(그 시점 문서 기반 상세 테스트 정의) → 에셋 준비(매핑 따라
실제 링크 또는 동작 stub 생성) → unity-cli 로 씬·Hierarchy·월드 셋업 → 플레이 테스트(에셋 로딩·참조 검증) →
자동 수정 루프(17-E 경계) → `result.md`+`evidence/`. 검증은 `test-runner` 위임. **여러 번 반복**되는 작업이므로
재실행 시 기존 더미·실제 에셋을 보존하고 델타만 반영한다.
