...
namespace TDA.PB4.Faction
{
        ...
        IFactionStateCommandReceiver
    {
        ...
        private void Awake()
        {
            ...
            // Inspector 측 _factionDefinitions 측 본격 등록 (Bootstrap 측 진입 자료 측 동일 메서드 측 호출)
            ...
            if (_verboseLog)
                Debug.Log($"[WorldFactionStateManager] Awake — {_definitionMap.Count} Faction 등록 완료");
        ...
        // ════════════════════════════════════════════════════════
        // ★ v1.2 — Faction 등록 (Bootstrap 진입 자료 / E-05 / S2 신규)
        // ════════════════════════════════════════════════════════
        
        /// <summary>
        /// FactionDefinitionSO 측 본 매니저 측 등록.
        /// Inspector 측 `_factionDefinitions` 목록 측 Awake 측 자동 호출 +
        /// FactionBootstrap (E-05) 측 외부 호출 측 본 메서드 측 사용.
        /// 
        /// 본 메서드 측 idempotent — 이미 등록된 FactionId 측 재호출 시 false 반환 + 회피.
        /// 본격 초기 상태 측 LIFECYCLE_ACTIVE 측 1 비트 측 정합 (E-05 측 본격).
        /// </summary>
        /// <returns>true = 신규 등록 / false = null / 중복 / FactionId 비어있음</returns>
        public bool RegisterFaction(FactionDefinitionSO def)
        {
            ...
            {
                Debug.LogWarning(
                    $"[WorldFactionStateManager] FactionId 비어있음 — {def.name} 등록 건너뜀");
                return false;
            }
            
            // Awake 이전 측 호출 측 안전 — 본 Dictionary 측 lazy 초기화
            if (_definitionMap == null) _definitionMap = new Dictionary<string, FactionDefinitionSO>();
            if (_runtimeStates == null) _runtimeStates = new Dictionary<string, FactionStateBits>();
            
            if (_definitionMap.ContainsKey(def.FactionId))
            {
                if (_verboseLog)
                    Debug.Log(
                        $"[WorldFactionStateManager] {def.FactionId} 이미 등록 — 건너뜀 (idempotent)");
                return false;
            }
            
            _definitionMap[def.FactionId] = def;
            
            // 기본 초기 상태 — LIFECYCLE_ACTIVE 측 1 비트 정합 (다른 3 카테고리 측 0)
            var initialBits = new FactionStateBits
            {
                moodBits      = 0u,
                tacticalBits  = 0u,
            ...
            _runtimeStates[def.FactionId] = initialBits;
            
            if (_verboseLog)
                Debug.Log(
                    $"[WorldFactionStateManager] {def.FactionId} 등록 — Lifecycle=ACTIVE");
        ...
        // ════════════════════════════════════════════════════════
        // ★ E-07 (S2) — FactionDefinition 외부 조회 API
        // ════════════════════════════════════════════════════════
        
        /// <summary>
        /// 등록된 FactionDefinitionSO 조회 — ScenarioArcManager / 기타 매니저에서
        /// SpeechTriggerIdOnEnter / OnDecline / EscalationModifier 조회용.
        /// 미등록 시 null 반환.
        /// </summary>
        public FactionDefinitionSO GetDefinition(string factionId)
        {
            if (_definitionMap == null || string.IsNullOrEmpty(factionId)) return null;
            return _definitionMap.TryGetValue(factionId, out var def) ? def : null;
        }
        ...
        /// <summary>
        /// 모든 Faction 측 런타임 상태 측 ★ 초기 상태 (LIFECYCLE_ACTIVE = 1, 그 외 0) 측 reset.
        /// 본 reset 측 RegisterFaction 측 본격 동일 초기값. OnFactionStateChanged 이벤트 발행.
        /// 통계 카운터 측 보존.
        /// 디버그 / 테스트 측 reset 도구.
        /// </summary>
        ...
        public void ResetAllState()
        {
            ...
            // 본격 초기 상태 — RegisterFaction 측 본격 동일
            var initialBits = new FactionStateBits
            {
                moodBits      = 0u,
                tacticalBits  = 0u,
            ...
            foreach (var id in ids)
            {
                ...
                // 이미 초기 상태 측 동일 — 이벤트 발행 회피
                if (oldBits.moodBits      == initialBits.moodBits &&
                    oldBits.tacticalBits  == initialBits.tacticalBits &&
                    oldBits.lifecycleBits == initialBits.lifecycleBits &&
                    oldBits.relationBits  == initialBits.relationBits)
                    continue;
                
                _runtimeStates[id] = initialBits;
                RaiseStateChanged(id, oldBits, initialBits);
            }
            
            Debug.Log(
                $"[WorldFactionStateManager] All {ids.Count} Faction(s) state reset → Lifecycle=ACTIVE.");
        }
