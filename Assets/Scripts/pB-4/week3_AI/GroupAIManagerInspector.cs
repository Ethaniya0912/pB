// =============================================================================
// GroupAIManagerInspector.cs  |  pB-4 Wk3 CustomEditor v3.7
...
//
// [v3.7 — 2026-05-12]
//   ★ BaseAIBrain 일반화 대응 (Mob + Humanoid 모두 등록 가능):
//     - LastTokenWinner ObjectField: typeof(MobAIBrain) → typeof(BaseAIBrain)
//     - m.brain.currentTarget → (m.brain as MobAIBrain)?.currentTarget cast
//       (Humanoid 는 currentTarget 필드 별도 디자인 — Wk5+)
// =============================================================================
#if UNITY_EDITOR
    ...
    public class GroupAIManagerInspector : UnityEditor.Editor
    {
        ...
        private void DrawGroupStatus(GroupAIManager grp)
        {
            ...
            using (new EditorGUI.DisabledScope(true))
            {
                ...
                // ★ v3.7 — LastTokenWinner = BaseAIBrain (Mob + Humanoid 모두)
                EditorGUILayout.ObjectField("Last Token Winner", grp.LastTokenWinner,
                    typeof(BaseAIBrain), true);
        ...
        private void DrawMembers(GroupAIManager grp)
        {
            ...
            using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar))
            {
                ...
                GUILayout.Label("Type", _tableHeaderStyle, GUILayout.Width(50));   // ★ v3.7 — Type 컬럼 추가
            ...
            foreach (var m in grp.Members)
            {
                ...
                // ★ v3.7 — MobAIBrain cast (currentTarget 등 Mob 특정 멤버 접근용)
                ...
                using (new EditorGUILayout.HorizontalScope(rowStyle))
                {
                    ...
                    // ★ v3.7 — Type 표시 (Mob / Humanoid / ?)
                    Color prevColor = GUI.color;
                    string brainType;
                    if (mob != null) { brainType = "Mob"; GUI.color = new Color(1.0f, 0.7f, 0.4f); }
                    else if (m.brain is TDA.PB4.AI.Humanoid.HumanoidAIBrain) { brainType = "Humanoid"; GUI.color = new Color(0.4f, 0.85f, 1.0f); }
                    else { brainType = "?"; GUI.color = Color.gray; }
                    GUILayout.Label(brainType, rowStyle, GUILayout.Width(50));
                    GUI.color = prevColor;
                    ...
                    // ★ v3.7 — Mob 만 currentTarget (Humanoid 은 "(N/A)" 표시)
                    Transform brainTgt = mob != null ? mob.currentTarget : null;
                    string targetName;
                    if (mob == null) { targetName = "(N/A)"; GUI.color = new Color(0.5f, 0.5f, 0.5f); }
                    else if (brainTgt == null) { targetName = "—"; GUI.color = new Color(0.6f, 0.6f, 0.6f); }
                    else { targetName = brainTgt.name; }
                    ...
                    if (brainTgt != null)
                    {
                        float d = Vector3.Distance(m.brain.transform.position, brainTgt.position);
                        distStr = $"{d:F1}m";
                    }
        ...
        private static Color ColorForBTState(string state)
        {
            switch (state)
            {
                case "Attack": return new Color(1.0f, 0.5f, 0.5f);
                case "Flee": return new Color(1.0f, 0.85f, 0.3f);
                case "Patrol": return new Color(0.6f, 0.85f, 1.0f);
